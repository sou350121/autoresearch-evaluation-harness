...
FIX_SUMMARIES = {
    "normalize_local_ui_url_to_loopback": "Normalize wildcard and empty UI hosts to 127.0.0.1 so CLI URLs match browser-facing doctor URLs.",
}


class DeepScientistLocalUiUrlAdapter(TaskAdapter):
    name = "deepscientist_local_ui_url_demo"
    ...
    def __init__(
        ...
        *,
        ...
        self.root = root
        self.demo_root = root / "demo_deepscientist_local_ui_url"
        self.target_path = target_path or (self.demo_root / "ui_url.py")
    ...
    def propose(self, accepted: AcceptedState, history: list[dict[str, str]], revision: int) -> Proposal:
        ...
        if self.proposer_name == "llm_codex":
            ...
            catalog = {fix_id: FIX_SUMMARIES[fix_id] for fix_id in missing or ["normalize_local_ui_url_to_loopback"]}
            selected, llm_metadata = llm_proposer.select_fix_ids(
                ...
                memory_summary="Local UI URL demo has no reusable method memory." if self.llm_memory_enabled else None,
            )
        else:
            llm_metadata = {}
            selected = missing[: self._fix_budget()] if missing else ["normalize_local_ui_url_to_loopback"]
        return Proposal(
            summary=f"{self.proposer_name} deepscientist local ui url proposal",
            scope_label=self.scope_label,
    ...
    def retry_after_reject(
        ...
        if not missing:
            return None
        return Proposal(
            summary=f"{self.proposer_name} deepscientist local ui url retry proposal",
            scope_label=self.scope_label,
