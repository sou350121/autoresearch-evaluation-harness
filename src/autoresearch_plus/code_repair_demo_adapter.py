...
FIXES = {
    "fix_tax": ("return subtotal * tax_rate", "return subtotal * (1.0 + tax_rate)"),
    "fix_discount": ("return total + total * pct", "return total - total * pct"),
    "fix_strip": ("return int(raw)", "return int(raw.strip())"),
}


class CodeRepairDemoAdapter(TaskAdapter):
    name = "code_repair_demo"
    ...
    def __init__(
        ...
        *,
        ...
        self.root = root
        self.demo_root = root / "demo_code_repair"
        self.target_path = target_path or (self.demo_root / "calculator.py")
    ...
    def propose(self, accepted: AcceptedState, history: list[dict[str, str]], revision: int) -> Proposal:
        ...
        if self.proposer_name == "llm_codex":
            ...
            catalog = {fix_id: f"Apply {fix_id}" for fix_id in (missing or ["fix_tax"])}
            selected, llm_metadata = llm_proposer.select_fix_ids(
                ...
                memory_summary="Code-repair demo has no reusable method memory." if self.llm_memory_enabled else None,
            )
        else:
            llm_metadata = {}
            selected = missing[: self._fix_budget()] if missing else ["fix_tax"]
        return Proposal(
            summary=f"{self.proposer_name} code-repair proposal",
            scope_label=self.scope_label,
            metadata={
    ...
    def retry_after_reject(
        ...
        if not missing:
            return None
        return Proposal(
            summary=f"{self.proposer_name} code-repair retry proposal",
            scope_label=self.scope_label,
            metadata={
                "proposal_kind": self.proposer_name,
                "fix_ids": missing[: self._fix_budget()],
                "retry_attempt": 2,
            },
        )
    ...
    def evaluate(self) -> EvalResult:
        ...
        sys.modules.pop("test_calculator", None)
